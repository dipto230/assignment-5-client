...
const Navbar = () => {
  ...
  // ✅ ADDED: AUTO SUGGESTION (debounce)
  useEffect(() => {
    if (!search) {
      setSuggestions([]);
      return;
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages([...newMessages, { role: "ai", text: data.data }]);
    } catch (err) {
      console.error(err);
    }
                ...
                {suggestions.map((lawyer) => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      if (!id) {
                        console.error("ID missing:", lawyer);
                        return;
                      }

                      router.push(`/consultation/lawyer/${id}`); // ✅ FIXED
                      setShowSuggestions(false); // ✅ UX
                      setSearch(""); // ✅ UX
                    }}
