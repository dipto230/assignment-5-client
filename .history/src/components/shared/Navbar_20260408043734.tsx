...
const Navbar = () => {
  ...
  // 🔍 AI SEARCH (manual enter)
  const handleSearch = async () => {
    if (!search) return;

    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/ai/search`,
        {
          method: "POST",
        ...
        <div className="relative">
          {loading ? null : user ? (
            <>
              ...
              >
                {user?.image ? (
                  <Image src={user.image} width={40} height={40} alt="avatar" />
                ) : (
                  getInitial()
                )}
              </button>
              ...
              {open && (
                ...
                >
                  <div className="px-4 py-2 text-xs border-b">{user.role}</div>
            ...
            <div className="flex gap-4">
              <Link href="/login">
                <button className="px-4 py-1.5 border rounded">Login</button>
              </Link>
              <Link href="/register">
                <button className="px-4 py-1.5 bg-blue-600 text-white rounded">Register</button>
              </Link>
            </div>
          )}
        </div>
