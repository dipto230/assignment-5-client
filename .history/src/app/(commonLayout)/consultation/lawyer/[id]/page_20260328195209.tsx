    ...
    const fetchData = async () => {
      try {
        const [lawyerRes, scheduleRes] = await Promise.all([
          ...
          fetch(`https://assignment-5-backend-sepia.vercel.app/api/v1/lawyer-schedules?lawyerId=${id}`, {
            credentials: "include",
          }),
        ]);
  ...
  const handleBook = async () => {
    ...
    try {
      const res = await fetch(
        "https://assignment-5-backend-sepia.vercel.app/api/v1/appointments/book-appointment",
        {
          method: "POST",
          headers: {
