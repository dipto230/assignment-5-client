...
import { appointmentService } from "@/services/appointment.service"; // create this service
import {
  Table,
  TableBody,
  TableCell,
...
export default function AdminAppointments() {
  ...
  // Change appointment status (optional)
  const statusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) =>
      appointmentService.changeStatus(id, status),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["appointments", page] }),
  });
                    ...
                    <Button
                      ...
                      onClick={() =>
                        statusMutation.mutate({
                          id: a.id,
                          status: a.status === "SCHEDULED" ? "COMPLETED" : "SCHEDULED",
                        })
                      }
                    >
                      Toggle Status
