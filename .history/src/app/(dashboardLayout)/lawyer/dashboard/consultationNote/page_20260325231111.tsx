...
export default function ConsultationNotesPage() {
  ...
  const { data: notes = [], isLoading, isError, refetch } = useQuery<ConsultationNote[]>({
    ...
    enabled: !!user?.id,
    queryFn: async () => {
      const res = await apiClient.get("/api/v1/consultation-notes/my-notes");
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const res = await apiClient.post("/api/v1/consultation-notes", newNote);
      return res.data.data;
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      const res = await apiClient.patch(`/api/v1/consultation-notes/${editNoteId}`, editNoteData);
      return res.data.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["myConsultationNotes", user?.id] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await apiClient.delete(`/api/v1/consultation-notes/${id}`);
      return res.data;
    },
